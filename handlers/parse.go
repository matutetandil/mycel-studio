...
// DoParse is the core parse logic, usable from both HTTP handlers and Wails bindings.
func DoParse(req *models.ParseRequest) (*models.ParseResponse, error) {
	ctx := context.Background()
	hclParser := parser.NewParser()
	...
	if req.Path != "" {
		...
		if statErr != nil {
			return &models.ParseResponse{
				Success: false,
				Errors:  []models.ParseError{{Message: "Path not found: " + req.Path}},
			}, nil
		}
		if info.IsDir() {
			config, err = hclParser.Parse(ctx, req.Path)
		...
		return &models.ParseResponse{
			Success: false,
			Errors:  []models.ParseError{{Message: "No content, files, or path provided"}},
		}, nil
	}
	...
	if err != nil {
		return &models.ParseResponse{
			Success: false,
			Errors:  []models.ParseError{{Message: err.Error()}},
		}, nil
	}
